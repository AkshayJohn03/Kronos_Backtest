...
st.markdown("### Audited Out-of-Sample Backtesting, Technical Moving Averages & Live Session Analytics (IST)")
...
run_version = st.sidebar.radio("Select Backtest Session:", [
    "🌅 Afternoon Session (02:00 PM to 03:30 PM IST Close - With 20 EMA & 50 SMA)",
...
if "Afternoon Session" in run_version:
    st.subheader(f"🌅 Afternoon Session (02:00 PM to 03:30 PM IST Market Close) - {timeframe}")
    st.markdown("Includes **20 EMA (Cyan line)** and **50 SMA (Purple line)** trend overlays alongside **Kronos AI Forecast (Gold Dashed line)**.")

    img_pm = os.path.join(plots_pm, f"afternoon_session_{tf_file_key}.png")
    if os.path.exists(img_pm):
        st.image(Image.open(img_pm), use_column_width=True, caption=f"Afternoon Session Candlestick Forecast ({timeframe})")
    ...
    if os.path.exists(run3_csv):
        st.dataframe(pd.read_csv(run3_csv), use_container_width=True)
...
st.markdown("---")
st.info("💡 **Moving Average Rule:** When 20 EMA is above 50 SMA and Kronos predicts an upward move, conviction for ATM Call Options is significantly higher.")
